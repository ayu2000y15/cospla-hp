...
class AdminContentSchemaController extends Controller
{
    ...
    private function parseArrayItems($arrayItemsData)
    {
        ...
        $options = $arrayItemsData['options'] ?? [];
        ...
        foreach ($names as $index => $name) {
            ...
            // selectタイプまたはradioタイプの場合は選択肢を追加
            if (($type === 'select' || $type === 'radio') && isset($options[$index]) && !empty($options[$index])) {
                $item['options'] = $this->parseOptions($options[$index]);
            }
