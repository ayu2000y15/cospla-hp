                                                            ...
                                                            @if(isset($field['array_items']) && is_array($field['array_items']))
                                                                @foreach($field['array_items'] as $arrayItem)
                                                                    <div class="mb-2">
                                                                        ...
                                                                        @elseif($arrayItem['type'] == 'textarea')
                                                                            <textarea
                                                                                name="{{ $field['col_name'] }}[{{ $index }}][{{ $arrayItem['name'] }}]"
                                                                                class="form-control"
                                                                                rows="4">{{ $item[$arrayItem['name']] ?? '' }}</textarea>
                                                                        @elseif($arrayItem['type'] == 'number')
                                                                            <input type="number"
                                                                                name="{{ $field['col_name'] }}[{{ $index }}][{{ $arrayItem['name'] }}]"
                                                                                class="form-control"
                                                                                value="{{ $item[$arrayItem['name']] ?? '' }}">
                                                                        @elseif($arrayItem['type'] == 'email')
                                                                            <input type="email"
                                                                                name="{{ $field['col_name'] }}[{{ $index }}][{{ $arrayItem['name'] }}]"
                                                                                class="form-control"
                                                                                value="{{ $item[$arrayItem['name']] ?? '' }}">
                                                                        @elseif($arrayItem['type'] == 'tel')
                                                                            <input type="tel"
                                                                                name="{{ $field['col_name'] }}[{{ $index }}][{{ $arrayItem['name'] }}]"
                                                                                class="form-control"
                                                                                value="{{ $item[$arrayItem['name']] ?? '' }}">
                                                                        @elseif($arrayItem['type'] == 'select')
                                                                            <select name="{{ $field['col_name'] }}[{{ $index }}][{{ $arrayItem['name'] }}]" class="form-select">
                                                                                <option value="">選択してください</option>
                                                                                @if(isset($arrayItem['options']) && is_array($arrayItem['options']))
                                                                                    @foreach($arrayItem['options'] as $option)
                                                                                        <option value="{{ $option['value'] }}" {{ isset($item[$arrayItem['name']]) && $item[$arrayItem['name']] == $option['value'] ? 'selected' : '' }}>
                                                                                            {{ $option['label'] }}
                                                                                        </option>
                                                                                    @endforeach
                                                                                @endif
                                                                            </select>
                                                                        @elseif($arrayItem['type'] == 'radio')
                                                                                    ...
                                                                                    @foreach($arrayItem['options'] as $option)
                                                                                        <div class="form-check">
                                                                                            <input type="radio"
                                                                                                ...
                                                                                                id="{{ $field['col_name'] }}_{{ $index }}_{{ $arrayItem['name'] }}_{{ $option['value'] }}"
                                                                                                name="{{ $field['col_name'] }}[{{ $index }}][{{ $arrayItem['name'] }}]"
                                                                                                value="{{ $option['value'] }}"
                                                                                                {{ isset($item[$arrayItem['name']]) && $item[$arrayItem['name']] == $option['value'] ? 'checked' : '' }}>
                                                                                            <label class="form-check-label" for="{{ $field['col_name'] }}_{{ $index }}_{{ $arrayItem['name'] }}_{{ $option['value'] }}">
                                                                                                {{ $option['label'] }}
                                                                                            </label>
                                                                                        </div>
                                                                                    @endforeach
                                                                        ...
                                                                        @elseif($arrayItem['type'] == 'date')
                                                                            <input type="date"
                                                                                ...
                                                                                class="form-control"
                                                                                value="{{ $item[$arrayItem['name']] ?? '' }}">
                                                                        @elseif($arrayItem['type'] == 'month')
                                                                            <input type="month"
                                                                                name="{{ $field['col_name'] }}[{{ $index }}][{{ $arrayItem['name'] }}]"
                                                                                class="form-control"
                                                                                value="{{ $item[$arrayItem['name']] ?? '' }}">
                                                                        @elseif($arrayItem['type'] == 'file')
                                                                            <div class="array-file-upload-container">
                                                                                <input type="file"
                                                                                    id="{{ $field['col_name'] }}_{{ $index }}_{{ $arrayItem['name'] }}"
                                                                                    name="{{ $field['col_name'] }}[{{ $index }}][{{ $arrayItem['name'] }}]"
                                                                                    class="form-control array-file-input">
                                                                                @if(!empty($item[$arrayItem['name']]))
                                                                                    <div class="mt-2">
                                                                                        <div class="d-flex align-items-center">
                                                                                            <img src="{{ asset($item[$arrayItem['name']]) }}" class="img-thumbnail me-2" style="max-width: 100px; max-height: 100px;">
                                                                                            <div>
                                                                                                <p class="mb-1">現在のファイル</p>
                                                                                                <input type="hidden" name="{{ $field['col_name'] }}[{{ $index }}][{{ $arrayItem['name'] }}_current]" value="{{ $item[$arrayItem['name']] }}">
                                                                                                <button type="button" class="btn btn-sm btn-outline-danger array-file-delete-btn">
                                                                                                    <i class="fas fa-times"></i> 削除
                                                                                                </button>
                                                                                            </div>
                                                                                        </div>
                                                                        ...
                                                                        @elseif($arrayItem['type'] == 'files')
                                                                            <div class="array-files-upload-container">
                                                                                <input type="file"
                                                                                    ...
                                                                                    multiple>
                                                                                @if(!empty($item[$arrayItem['name']]) && is_array($item[$arrayItem['name']]))
                                                                                    <div class="mt-2">
                                                                                        <p>現在のファイル:</p>
                                                                                        <div class="d-flex flex-wrap gap-2">
                                                                                            @foreach($item[$arrayItem['name']] as $fileIndex => $filePath)
                                                                                                <div class="array-file-item">
                                                                                                    <img src="{{ asset($filePath) }}" class="img-thumbnail" style="width: 80px; height: 80px; object-fit: cover;">
                                                                                                    <input type="hidden" name="{{ $field['col_name'] }}[{{ $index }}][{{ $arrayItem['name'] }}_current][]" value="{{ $filePath }}">
                                                                                                    <button type="button" class="btn btn-sm btn-danger array-file-item-delete-btn" data-index="{{ $fileIndex }}">
                                                                                                        <i class="fas fa-times"></i>
                                                                                                    </button>
                                                                                                </div>
                                                                                            @endforeach
...
<style>
    ...
    .array-file-item .img-thumbnail {
        margin-bottom: 5px;
    }

    .array-file-item-delete-btn {
        position: absolute;
        top: -5px;
        right: -5px;
        padding: 0;
...
document.addEventListener('DOMContentLoaded', function() {
            ...
            try {
                ...
                if (arrayItems.length > 0) {
                    arrayItems.forEach(arrayItem => {
                        ...
                        itemHtml += `<div class="d-flex flex-wrap gap-3">`;
    ...
    if (e.target.closest('.array-file-delete-btn')) {
        ...
        const fileContainer = deleteBtn.closest('.array-file-upload-container');
    ...
    function updateArrayItemIndexes(fieldName) {
        ...
        items.forEach((item, index) => {
            ...
            radioButtons.forEach(radio => {
                if (radio.id && radio.id.includes(`${fieldName}_${index}`)) {
