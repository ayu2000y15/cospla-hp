        ...
        document.addEventListener('DOMContentLoaded', function () {
            ...
            editButtons.forEach(button => {
                button.addEventListener('click', function () {
                    ...
                    // 選択肢データがある場合は表示
                    if (optionsData && (type === 'select' || type === 'radio')) {
                        try {
                            // HTMLエンティティをデコード
            ...
            // 配列項目を追加する関数
            function addArrayItem(itemName = '', itemType = 'text', itemOptions = '') {
                            ...
                            <div class="array-item card p-3 mb-2" id="${itemId}">
                                ...
                                <div class="row">
                                    ...
                                    <div class="col-md-6 mb-2">
                                        ...
                                        <select name="array_items[type][]" class="form-select array-item-type">
                                            <option value="text" ${itemType === 'text' ? 'selected' : ''}>text (文字列)</option>
                                            <option value="textarea" ${itemType === 'textarea' ? 'selected' : ''}>textarea (複数行テキスト)</option>
                                            <option value="number" ${itemType === 'number' ? 'selected' : ''}>number (数字)</option>
                                            <option value="email" ${itemType === 'email' ? 'selected' : ''}>email (メールアドレス)</option>
                                            <option value="tel" ${itemType === 'tel' ? 'selected' : ''}>tel (電話番号)</option>
                                            <option value="date" ${itemType === 'date' ? 'selected' : ''}>date (日付[年月日])</option>
                                            <option value="month" ${itemType === 'month' ? 'selected' : ''}>month (日付[年月])</option>
                                            <option value="select" ${itemType === 'select' ? 'selected' : ''}>select (プルダウン)</option>
                                            <option value="radio" ${itemType === 'radio' ? 'selected' : ''}>radio</option>
                                            <option value="boolean" ${itemType === 'boolean' ? 'selected' : ''}>boolean (真偽値)</option>
                                            <option value="url" ${itemType === 'url' ? 'selected' : ''}>url (URL)</option>
                                            <option value="file" ${itemType === 'file' ? 'selected' : ''}>file (単一ファイル)</option>
                                            <option value="files" ${itemType === 'files' ? 'selected' : ''}>files (複数ファイル)</option>
                                        </select>
                                    </div>
                                </div>
                                <div class="array-item-options mt-2" style="display: ${(itemType === 'select' || itemType === 'radio') ? 'block' : 'none'}">
                                    <label class="form-label">選択肢</label>
                                    <textarea name="array_items[options][]" class="form-control" rows="3" placeholder="各行に「値:表示名」の形式で入力してください。">${itemOptions || ''}</textarea>
                                    <div class="form-text">
                                        各行に「値:表示名」の形式で入力してください。表示名を省略すると値がそのまま表示されます。<br>
                                        例: <code>male:男性</code>, <code>female:女性</code>, <code>other:その他</code>
                                    </div>
            ...
            function loadArrayItems(items) {
                ...
                items.forEach(item => {
                    ...
                    addArrayItem(item.name, item.type, optionsText);
                });
            }

            // 配列項目のタイプが変更されたときの処理
            document.addEventListener('change', function (e) {
                if (e.target.classList.contains('array-item-type')) {
                    const optionsContainer = e.target.closest('.array-item').querySelector('.array-item-options');
                    if (optionsContainer) {
                        optionsContainer.style.display = (e.target.value === 'select' || e.target.value === 'radio') ? 'block' : 'none';
                    }
                }
            });
        });
