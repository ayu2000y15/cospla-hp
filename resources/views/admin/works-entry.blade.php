...
@section('content')
    <div class="space-y-10">
        <h1 class="text-2xl font-semibold text-gray-900">WORKSページ編集</h1>
        ...
        <details class="bg-white rounded-lg shadow group">
            ...
            <div class="p-6 border-t border-gray-200">
                <form action="{{ route('admin.works.client.store') }}" method="POST" class="space-y-6">
                    @csrf
        ...
        <div class="p-6 bg-white rounded-lg shadow">
            <h3 class="text-lg font-medium leading-6 text-gray-900">登録済みグループ一覧</h3>
            <p class="mt-1 text-sm text-gray-500">ドラッグ＆ドロップでグループの表示順を変更できます。</p>
            <div id="client-list" class="mt-4 space-y-4">
                @forelse ($clients as $client)
                    <details class="p-4 bg-gray-50 border rounded-lg shadow-sm group" data-id="{{ $client->client_id }}">
                        ...
                        <div class="mt-4 border-t pt-4" x-data="{ isEditing: false }">
                            {{-- 表示モード --}}
                            <div x-show="!isEditing">
                                <div class="md:flex md:justify-between md:items-start">
                                    ...
                                    <div class="flex items-start space-x-2 flex-shrink-0">
                                        ...
                                        <form action="{{ route('admin.works.client.destroy', $client) }}" method="POST"
                            ...
                            {{-- 編集モード --}}
                            <form x-show="isEditing" x-cloak action="{{ route('admin.works.client.update', $client) }}" method="POST"
                                class="space-y-6">
                                @csrf @method('PUT')
                                <div class="grid grid-cols-1 gap-6 sm:grid-cols-2">
                            ...
                            <hr class="my-4">
                            <div class="p-4 mt-4 bg-white border border-gray-200 rounded-lg">
                                <h4 class="text-base font-medium text-gray-700">画像を追加</h4>
                                <form action="{{ route('admin.works.images.upload') }}" method="POST" enctype="multipart/form-data"
                                    class="mt-4">
                                    @csrf
                            ...
                            <div class="mt-6">
                                @if($client->images->isNotEmpty())
                                    <h5 class="text-base font-medium text-gray-700">登録済み画像</h5>
                                    <p class="mt-1 text-sm text-gray-500">各画像の表示優先順位を数値で設定できます。数値が小さいほど先に表示されます。</p>
                                    <div class="grid grid-cols-2 mt-4 gap-x-4 gap-y-8 sm:grid-cols-3 lg:grid-cols-4 xl:gap-x-8">
                                        @foreach ($client->images as $image)
                                            <div class="relative group">
                                                ...
                                                <form action="{{ route('admin.works.image.destroy', $image) }}" method="POST"
                                                ...
                                                <form action="{{ route('admin.works.image.update', $image) }}" method="POST" class="mt-2">
                                                     @csrf @method('PUT')
                                                     <div class="flex items-center gap-2">
                                                         <input type="number" name="priority" value="{{ $image->priority }}" class="block w-20 p-1 text-sm border-gray-300 rounded-md shadow-sm">
                                                         <button type="submit" class="px-2 py-1 text-xs font-medium text-white bg-indigo-600 rounded-md hover:bg-indigo-700">更新</button>
                                                     </div>
                                                </form>
                                            </div>
                                        @endforeach
            ...
            if (clientList) {
                new Sortable(clientList, {
                    ...
                    onEnd: function (evt) {
                        ...
                        fetch('{{ route('admin.works.client.reorder') }}', {
                            method: 'POST',
                            headers: {
