...
<main>
    ...
    <div class="container">
        <div class="container-box news-page">
            ...
            @foreach ($newsImgList as $img)
                @if($newsItem->NEWS_ID == $img->NEWS_ID)
                <img class="news-photo" src="{{ asset($img->FILE_PATH . $img->FILE_NAME) }}"
                alt="{{ $img->COMMENT }}" onclick="openImagePreview('{{ asset($img->FILE_PATH . $img->FILE_NAME) }}', '{{ $img->COMMENT }}')">
                @endif
            @endforeach
            </div>
        </div>
    ...
    <div class="preview-overlay" id="imagePreview">
        ...
        <button class="preview-arrow preview-prev-arrow" id="prevImage" onclick="changePreviewImage(-1)">&#10094;</button>
        <button class="preview-arrow preview-next-arrow" id="nextImage" onclick="changePreviewImage(1)">&#10095;</button>
    </div>
</main>
@endsection
