<div class="flex flex-1 flex-col overflow-y-auto pt-5 pb-4">
    ...
    <nav class="mt-5 flex-1 space-y-1 px-2">
        <a href="{{ route('admin') }}?tab=talent-list"
            class="{{ !request()->routeIs('admin.works.*') && request()->query('tab', 'talent-list') == 'talent-list' ? 'bg-gray-900 text-white' : 'text-gray-300 hover:bg-gray-700 hover:text-white' }} group flex items-center px-2 py-2 text-sm font-medium rounded-md">タレント管理</a>
        ...
        <a href="{{ route('admin.works.index') }}"
            class="{{ request()->routeIs('admin.works.*') ? 'bg-gray-900 text-white' : 'text-gray-300 hover:bg-gray-700 hover:text-white' }} group flex items-center px-2 py-2 text-sm font-medium rounded-md">WORKS管理</a>
